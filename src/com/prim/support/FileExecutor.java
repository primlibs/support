...
public class FileExecutor {
    ...
    /**
     * возвращает содержимое файла в виде строки. Может работать некорректно
     * из-за проблем с кодировкой.
     *
    ...
    /**
     * возвращает содержимое файла в виде строки. Если charset == null, то
     * читает содержимое файла без учета кодировки. В этом случае может работать
     * некорректно.
     *
    ...
    /**
     * записать строку в файл. Перед записью содержимое файла стирается <br/>
    ...
    /**
     * добавить строку в файл. Содержимое файла не стирается, строка записываетс
     * в конец файла<br/> Если файла не существует - он будет создан
     *
     * @param string
     * @return
     */
    ...
    /**
     * копировать файл в новую директорию. Если в новой директории уже есть файл
     * с таким именем, то он стирается
     *
    ...
    /**
     * 
     * @param path fuul path to directory, this value do not set in object
     * @param execute 
     * @param read
     * @param write
     * @return 
     */
    public Boolean createDirectory(String path, Boolean execute, Boolean read, Boolean write){
        Boolean status=true;
        try{
            ...
            if(status==true){
                if(execute==true&&folder.canExecute()==false){
                   throw new Exception("Cant set dir exec");
                }
                if(read==true&&folder.canRead()==false){
                   throw new Exception("Cant set dir read");
                }
                if(write==true&&folder.canWrite()==false){
                   throw new Exception("Cant set dir write");
                }
            }else{
               errors.add("Cant create file");
            }   
        }catch(Exception e){
            status=false;
